...
angular.module('flowableModeler')
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', '$window', 'editorManager',
        ...
        $scope.editorFactory.promise.then(function () {
            ...
            $http({method: 'POST', url: "/getCategoryTree"}).success(function (data, status, headers, config) {
                console.log("--------------")
                console.log(data)
                $rootScope.categoryTree = data.data.menuList;
            })
            $http({method: 'GET', url: "/flowable/httpUrl/list?limit=100&offset=0"}).success(function (data, status, headers, config) {
                console.log("--------------")
                console.log(data)
                $rootScope.httpData = data.rows;
            })
            $http({method: 'GET', url: "/flowable/extClass/list?limit=100&offset=0"}).success(function (data, status, headers, config) {
                console.log("--------------")
                console.log(data)
                $rootScope.classData = data.rows;
            })
            $http({method: 'GET', url: "/flowable/extDatasource/list?limit=100&offset=0"}).success(function (data, status, headers, config) {
                console.log("--------------")
                console.log(data)
                $rootScope.dataSourceData = data.rows;
            })
        ...
        $scope.getPropertyWriteModeTemplateUrl = function (index) {
            ...
            if($scope.selectedItem.properties[index].key=="oryx-process_namespace"){
                return "editor-app/configuration/properties/target-namespace-property-write-template.html";
            }else if($scope.selectedItem.properties[index].key=="oryx-httptaskrequesturl"){
                return "editor-app/configuration/properties/target-http-property-write-template.html";
            }else if($scope.selectedItem.properties[index].key=="oryx-servicetaskclass"){
            	return "editor-app/configuration/properties/target-serviceclass-property-write-template.html";
            }else if($scope.selectedItem.properties[index].key=="oryx-servicetaskdelegateexpression"){
            	return "editor-app/configuration/properties/target-dbsource-property-write-template.html";
            }
